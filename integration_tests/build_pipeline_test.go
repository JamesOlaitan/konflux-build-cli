...
func TestBuildPipeline(t *testing.T) {
	...
	defer cleanupContainerStorageDir(workspaceDirHost)
	...
	t.Run("build-image-index", func(t *testing.T) {
		...
		for _, tag := range imageIndexTags {
			exists, err := CheckTagExistence(imageRegistry, outputImageName, tag)
			Expect(err).ToNot(HaveOccurred())
			Expect(exists).To(BeTrue())
		}
	...
	t.Run("apply-tags", func(t *testing.T) {
		...
		for _, tag := range []string{newTag, newTagFromLabel} {
			tagExists, err := CheckTagExistence(imageRegistry, outputImageName, tag)
	...
	t.Run("push-dockerfile", func(t *testing.T) {
		...
		tagExists, err := CheckTagExistence(imageRegistry, outputImageName, expectedContainerfileTag)
		...
		verifyContainer := NewBuildCliRunnerContainer("kbc-verify-containerfile-artifact", TaskRunnerImage)
		err = verifyContainer.StartWithRegistryIntegration(imageRegistry)
		Expect(err).ToNot(HaveOccurred())
		t.Cleanup(func() { verifyContainer.DeleteIfExists() })

		skopeoArgs := []string{"inspect", "--raw", "docker://" + pushContainerfileResults.ImageRef}
		manifestJson, _, err := verifyContainer.ExecuteCommandWithOutput("skopeo", skopeoArgs...)
		Expect(err).ToNot(HaveOccurred(), "failed to inspect containerfile artifact manifest")
